    ...
    if(item.deliverOrderState == "SPLITTED"){
      return (<Col xs={{ span: 16, offset: 1}} lg={{ span: 10, offset: 2}}><Steps current={parseInt(item.deliverOrderState)} className={styles.card}>
          ...
          <Steps current={getIndexByTitle(EnumDeliveryStatus, item.deliverOrderState)} className={styles.card}>
            <Step title="已创建" description={getIndexByTitle(EnumDeliveryStatus, item.deliverOrderState)>=0?<div><div>{`货物体积(m³)：${item.cube}`}</div><div>{`货单金额(元)：${item.price}`}</div><div>{`${item.createTime}`}</div></div>:''} />
            <Step title="已分配" description={getIndexByTitle(EnumDeliveryStatus, item.deliverOrderState)>0?<div><div>{`分配车辆：${item.vehicle.number}`}</div><div>{`司机：${item.driver.name}`}</div><div>{`司机电话：${item.driver.phone}`}</div><div>{`${item.distributTime}`}</div></div>:''} />
            <Step title="已接货" description={getIndexByTitle(EnumDeliveryStatus, item.deliverOrderState)>2?<div><div>{`发货人：${item.from.name} ${item.from.phone}`}</div><div>{`发货地址：${item.from.district} ${item.from.address}`}</div><div>{`${item.loadTime}`}</div></div>:''} />
            <Step title="已完成" description={getIndexByTitle(EnumDeliveryStatus, item.deliverOrderState)>4?<div><div>{`收货人：${item.to.name} ${item.to.phone}`}</div><div>{`收货地址：${item.to.district} ${item.to.address}`}</div><div>{`${item.arriveTime}`}</div></div>:''} />
          </Steps>
      )
  }
